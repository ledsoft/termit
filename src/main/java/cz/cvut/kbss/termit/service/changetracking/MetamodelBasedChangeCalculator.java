...
public class MetamodelBasedChangeCalculator implements ChangeCalculator {
    ...
    private Optional<UpdateChangeRecord> resolveTypesChange(Asset original, Asset update,
        ...
        final Object origTypes = getAttributeValue(typesSpec, original);
        final Object updateTypes = getAttributeValue(typesSpec, update);
        if (Objects.equals(origTypes, updateTypes)) {
            return Optional.empty();
        } else {
            final UpdateChangeRecord record = createChangeRecord(assetId, URI.create(RDF.TYPE));
            if (origTypes != null) {
                record.setOriginalValue(((Collection<?>) origTypes).stream().map(t -> URI.create(t.toString()))
                                                                   .collect(Collectors.toSet()));
            }
            if (updateTypes != null) {
                record.setNewValue(((Collection<?>) updateTypes).stream().map(t -> URI.create(t.toString()))
                                                                .collect(Collectors.toSet()));
            }
            return Optional.of(record);
        }
